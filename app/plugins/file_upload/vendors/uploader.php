...
class Uploader {
  ...
  function processFile($file = null){
    ...
    //if callback returns false halt the upload 
    if(!$fileName){
      //echo "fail: processFile [1]<br><br>";
       return false;
    }
    ...
    if($this->__moveUpload($this->file['tmp_name'], $target_path)){
      $this->finalFile = basename($target_path);
      //echo "success: processFile <br><br>";
      return $this->finalFile;
    }
    else{
      $this->_error(__('Unable to save temp file to file system.',true));
      //echo "fail: processFile [2]<br><br>";
      return false;
    }
  }
  ...
  */
  ...
  /* Due to the fact that we save file upload information before commit
   * (so that users don't have to re-upload their files if a validation
   * error occurs elsewhere in the form), we need to write our own file move
   * method.
   */
  private function __moveUpload($src, $dest) {
      $ret_condition = true;
      if(!copy($src, $dest)) {
          $ret_condition = false;
      }
      $this->__clearTempFiles($src);
