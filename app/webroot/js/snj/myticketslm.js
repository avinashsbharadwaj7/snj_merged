...
j(document).ready(function() {
  searchResultsTable = j("#searchResultsTable2").dataTable({
    ...
    "fnRowCallback": function(nRow, aData, iDisplayIndex, iRowIndex) {
              var txt = j("td:eq(5)", nRow).html();
              if (txt == "Yes")
              {
                 j("td:eq(5)", nRow).addClass("red-text"); // Conflict
              }
              txt = j("td:eq(6)", nRow).html();
              if (txt == "No")
              {
                 j("td:eq(6)", nRow).addClass("red-text"); // Resources assigned
              }
	      return nRow;
    }
  });
